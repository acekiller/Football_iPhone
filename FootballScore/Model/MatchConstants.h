//
//  MatchConstants.h
//  FootballScore
//
//  Created by qqn_pipi on 11-9-29.
//  Copyright 2011年 __MyCompanyName__. All rights reserved.
//

enum{    
    MATCH_SCORE_TYPE_ALL = 0,
    MATCH_SCORE_TYPE_FIRST = 1,
    MATCH_SCORE_TYPE_SINGLE = 2,
    MATCH_SCORE_TYPE_ZUCAI = 3,
    MATCH_SCORE_TYPE_JINGCAI = 4   
};

enum{    
    // match button tag, be careful to change the value together with button tag in UI
    MATCH_SELECT_STATUS_ALL = 11,              
    MATCH_SELECT_STATUS_NOT_STARTED = 12,
    MATCH_SELECT_STATUS_ON_GOING = 13,
    MATCH_SELECT_STATUS_FINISH = 14,
    MATCH_SELECT_STATUS_MYFOLLOW = 15
};



enum{
    
    // match button tag, be careful to change the value together with button tag in UI
   
 
    MATCH_DATA_STATUS_EVENT = 1,   
    MATCH_DATA_STATUS_LINEUP =2,
    MATCH_DATA_STATUS_ANALYSIS =3,
    MATCH_DATA_STATUS_ASIANODDS =4,
    MATCH_DATA_STATUS_AUROPEANODDS =5,
    MATCH_DATA_STATUS_SIZE =6




};